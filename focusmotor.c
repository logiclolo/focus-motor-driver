...
static struct timespec current_time, last_move_time;
...
static irqreturn_t motor_isr(int irq, void *dev_id)
{
    ...
    if (signal_pending (current)) {printk("P\n");};
#ifdef _DEBUG
	...
	{
		if ((current_time.tv_sec - last_move_time.tv_sec) > 5)
		{
			//DBG("[FocusMotor] MOTOR_ENABLE_PIN = 0\n");
			g_bMotorClose = TRUE;
	...
	else if (g_dwMotorStep > 0)
	{
		...
		last_move_time.tv_sec = current_time.tv_sec;
	}
						...
						unsigned int cmd, DWORD dwArg)
{
	...
	*-------------------------------------------------------------------*/
	...
	case MOTOR_SYS_LOG_LIST:
		...
		break;
	case MOTOR_TURN_OFF:
		// Deferred execute "Turn off motors for thermal issue"
		g_bMotorClose = FALSE;
		last_move_time.tv_sec = current_time.tv_sec;
		break;
	default:
